...
@login_required
def add_picture(request: HttpRequest) -> HttpResponse:
     PictureTagFormSet = modelformset_factory(ImageTag, form=ImageTagForm,
                                              extra=3)  # Adjust the extra parameter as needed
     if request.method == 'POST':
...
def era_search_results(request: HttpRequest, era: str) -> HttpResponse:
     if era:
          era_display_choices = {
